...
class RefundInstallmentOrder implements ShouldQueue
{
    ...
    {
        // 如果商品订单支付方式不是分期付款或者订单未支付或者订单退款状态为退款中，则不执行后面的逻辑
        ...
        if (!$installment = Installment::query()
        ...
        //判断当前所有还款计划是否全部退款成功,全部成功则将订单改为已退款
        ...
        switch ($item->payment_method) {
            case 'wechat':
                app('wechat_pay')->refund([
                    ...
                    'notify_url'     => ngrok_url('installments.wechat.refund_notify'),//可以直接传入回调地址，有效
                ]);
                // 将还款计划退款状态改成退款中
                $item->update([
            ...
            default:
                ...
                throw new SystemException('未知订单支付方式：'.$item->payment_method);
                break;
        }
    }
